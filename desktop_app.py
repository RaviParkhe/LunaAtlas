...
def launch_desktop():
    ...
    # Start FastAPI server thread if not already running
    import urllib.request
    server_ready = False
    for _ in range(15):
        try:
            res = urllib.request.urlopen(f"http://127.0.0.1:{PORT}/api/health", timeout=1)
            if res.status == 200:
                server_ready = True
                break
        except Exception:
    ...
    try:
        ...
        window = webview.create_window(
            title="LunaAtlas — Lunar South Pole Decision Support Workstation",
            url=url,
            width=1480,
            height=920,
            min_size=(1100, 720),
